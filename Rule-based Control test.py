...
control_data_path = 'data/rule_based_control_settings_flow1.xlsx'
...
def main():
    ...
    control_data = pd.read_excel(control_data_path)
    control_data = control_data.reset_index(drop=True)


    # 创建时间戳子文件夹
    current_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    result_folder = os.path.join("test results", f"Rule_based_Control_{test_filename}_{current_time}")
    ...
    for episode in range(len(test_data)):
        ...
        job, EQI, OCI, Power, OPEXplant_d, CSTR2_SNOx, Eff_TCOD, Eff_SNHx, Eff_TN = run_sim_multiple_pram(
            ...
            DO_setpoint, IMLR_setpoint, Carbon_setpoint
        )
...
def save_test_results_to_excel(results, result_folder):
    ...
    filename = os.path.join(
        result_folder,
        f"Rule_based_test_results_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    )
