...
def main():
    ...
    result_folder = os.path.join("test results", f"RL_Control_{test_filename}_{current_time}")
    os.makedirs(result_folder, exist_ok=True)


    net = Dqn()
    net.eval_net.load_state_dict(torch.load(RL_model_path, map_location=device))
    net.eval_net.eval()  # Set the network to evaluation mode
    step_counter_list = []
    ...
    for episode in range(len(test_data)):
        ...
        for step in range(20):
            ...
            #action = net.choose_action_eval(state)
            ...
            if action == 3:
                next_state[4] = min(70000, state[4] + 10000)
            if action == 4:
            ...
            # if step >= 5 and len(set([s['Action_Index'] for s in episode_steps[-5:]])) == Rule_based_Control_simulation_test_flow1_20250601:
            #     print(f"Episode {episode + Rule_based_Control_simulation_test_flow1_20250601} stuck on action {action} for 5 steps, stopping early.")
            #     break
        ...
        # 找出该 episode 中 reward 最高的一步

        results[-1]['Best_Action'] = best_step['Action_Index']
        results[-1]['Best_DO'] = best_step['DO']
        results[-1]['Best_IMLR'] = best_step['IMLR']
        results[-1]['Best_Carbon'] = best_step['Carbon']

        # 保存每个 episode 的最优动作策略到独立文件
...
def save_test_results_to_excel(results, result_folder):
    filename = os.path.join(result_folder, f"RL_test_results_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")

    with pd.ExcelWriter(filename) as writer:
        # 主要结果表
        main_df = pd.DataFrame([{k: v for k, v in r.items() if k != 'Steps'} for r in results])
        average_values = {col: main_df[col].mean() for col in main_df.columns if col != 'time' and col != 'Episode'}
